...
def eliminar_categoria(nombre_cat):
    ruta = Path(path_inicial, nombre_cat)
    if ruta.exists():
        if contar_cantidad_recetas_en_categoria(nombre_cat) == 0:
            os.rmdir(ruta)
...
def menu():
    archivo_abierto = Path('D:/Datos basura/menu.txt')
...
def contar_cantidad_recetas_en_categoria(carpeta):
    initial_count = 0
    for path in Path(carpeta).iterdir():
        if path.exists():
            initial_count += 1
